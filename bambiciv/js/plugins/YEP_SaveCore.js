...
Yanfly.Param.SaveVocabPlaytime = Number(Yanfly.Parameters['Playtime']);
Yanfly.Param.SaveVocabSaveCount = String(Yanfly.Parameters['Save Count']);
Yanfly.Param.SaveVocabGoldCount = Number(Yanfly.Parameters['Gold Count']);
...
Window_SaveInfo.prototype.drawPlaytime = function(dx, dy, dw) {
    ...
    if (Yanfly.Param.SaveVocabPlaytime > 0) {
      this.changeTextColor(this.systemColor());
	  //change by mtm101
      //this.drawText(Yanfly.Param.SaveVocabPlaytime, dx, dy, dw, 'left');
	  this.drawIcon(Yanfly.Param.SaveVocabPlaytime, dx + 2, dy + 2);
      this.changeTextColor(this.normalColor());
      this.drawText(text, dx, dy, dw, 'right');
    } else {
      this.drawText(text, dx, dy, dw, 'center');
...
Window_SaveInfo.prototype.drawGoldCount = function(dx, dy, dw) {
    ...
    if (Yanfly.Param.SaveVocabGoldCount > 0) {
      this.changeTextColor(this.systemColor());
	  //change by mtm101 to add icon support
      //var fmt = Yanfly.Param.SaveVocabGoldCount;
      //this.drawText(fmt.format(TextManager.currencyUnit), dx, dy, dw, 'left');
	  this.drawIcon(Yanfly.Param.SaveVocabGoldCount, dx + 2, dy + 2);
      this.changeTextColor(this.normalColor());
...
Window_SaveInfo.prototype.drawVariable = function(id, dx, dy, dw) {
    ...
    if (varName.length > 0) {
	  var is_emoti = varName.contains("\\i")
      this._drawData = true;
      this.changeTextColor(this.systemColor());
      dy += diff;
      this.drawTextEx(this.systemColorEx() + varName, dx, dy - (is_emoti ? 5 : 0), dw, 'left');
      dy -= diff;
      this.changeTextColor(this.normalColor());
      this._drawData = false;
      this.drawText(text, dx, dy, dw, 'right');
